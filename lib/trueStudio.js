...
async function _req({
  ...
  // ── Rate-limit back-off ────────────────────────────────────────────────
  if (r.status === 429 && _rl < 4) {
    const retryAfter = parseFloat(
      r.headers?.['retry-after'] || r.data?.retry_after || '5'
    );
    const waitMs = Math.ceil(retryAfter * 1000) + _gauss(400, 1500);
    await _sleep(waitMs);
    ...
    if (sitekey) {
      try {
        const solved = await solveCaptcha({
          ...
          rqdata, rqtoken, url, context: 'discord',
        });
        if (solved) {
          const nb = { ...(body || {}), captcha_key: solved };
          if (rqtoken) nb.captcha_rqtoken = rqtoken;
          return _req({ method, url, token, body: nb, netOpts, mfa, extra, _rl, _cap: _cap + 1, _mfa });
        }
      } catch (e) {
...
async function createApplication({ token, name, teamId = null, netOpts = {} }) {
  ...
  if (r.status >= 400) {
    ...
    if (r.status === 429) {
      ...
      throw new DiscordError(`Rate limit على إنشاء التطبيقات — انتظر ${retryAfter || '?'} ثانية`, { status: r.status, data: r.data, code: 'RATE_LIMITED' });
    }
...
async function resetBotToken({ token, appId, mfa = null, netOpts = {} }) {
  ...
  if (r.status >= 400) {
    ...
    throw new DiscordError('resetBotToken فشل: ' + msg, { status: r.status, data: r.data });
  }
...
async function setApplicationIntents({ token, appId, enabled = true, netOpts = {} }) {
  const app = await getApplication({ token, appId, netOpts });
  ...
  if (r.status >= 400) {
    ...
    if (r.status === 429) {
      const retryAfter = Number(r.headers?.['retry-after'] || r.data?.retry_after || 0);
      throw new DiscordError(`Rate limited while updating intents — retry after ${retryAfter || '?'}s`, {
...
async function addBotToGuild({ token, clientId, guildId, permissions = '8', netOpts = {} }) {
  ...
  if (r.status >= 400) {
    const msg = r.status === 401 ? 'Unauthorized — check account token'
      : r.status === 403       ? 'Missing permissions or bot already in server'
      : (r.data?.message || String(r.status));
    throw new DiscordError('addBotToGuild failed: ' + msg, { status: r.status, data: r.data });
  }
  return r.data;
}
...
async function updateBotProfile({ botToken, avatar = undefined, banner = undefined, netOpts = {} }) {
  ...
  const r = await client.http({
    ...
    data: body,
    headers: {
      'Authorization': botAuthHeader(botToken),
      'Content-Type': 'application/json',
