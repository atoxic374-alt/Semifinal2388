...
async function setApplicationPublic({ token, appId, enabled = true, netOpts = {}, app = null }) {
  app = app || await getApplication({ token, appId, netOpts });
  if (!!(app?.bot_public) === enabled) return app;
  const r = await _req({
    method: 'PATCH',
    url:    `${API}/applications/${appId}`,
    token,
    body:   { bot_public: enabled },
    netOpts,
    extra: { 'X-Context-Properties': CTX_DEV_PORTAL },
  });
  if (r.status >= 400) {
    ...
    if (r.status === 429) {
      const rateLimit = rateLimitInfoFromResponse(r);
      throw new DiscordError(`Rate limited while updating bot_public — retry after ${rateLimit.retryAfter || rateLimit.resetAfter || '?'}s`, {
        status: r.status, data: r.data, code: 'RATE_LIMITED',
        retryAfter: rateLimit.retryAfter || rateLimit.resetAfter || 0,
        rateLimit,
      });
    }
    throw new DiscordError('setApplicationPublic failed: ' + msg, { status: r.status, data: r.data, code });
  }
  return r.data;
}
...
async function updateAppVisuals({ token, appId, icon = undefined, coverImage = undefined, name = undefined, netOpts = {} }) {
  const body = {};
  if (icon       !== undefined) body.icon        = icon       || null;
  if (coverImage !== undefined) body.cover_image = coverImage || null;
  if (name       !== undefined && name !== null) body.name   = String(name).slice(0, 32) || undefined;
...
function cloneClientWithProxy(baseClient, proxyUrl) {
  ...
  if (proxyUrl && typeof proxyUrl === 'string' && proxyUrl.trim()) {
    ...
    if (scheme === 'socks' || scheme === 'socks4' || scheme === 'socks5' || scheme === 'socks5h') {
      const { SocksProxyAgent } = require('socks-proxy-agent');
      // keepAlive: false → every request opens a fresh TCP tunnel → new IP from rotating proxy
      const agent = new SocksProxyAgent(withScheme, { timeout: 30_000 });
      httpsAgent = agent;
      httpAgent  = agent;
    } else {
      const { HttpsProxyAgent } = require('https-proxy-agent');
      const { HttpProxyAgent }  = require('http-proxy-agent');
      // keepAlive: false → new TCP CONNECT per request → new IP from rotating proxy
      httpsAgent = new HttpsProxyAgent(withScheme, { keepAlive: false });
      httpAgent  = new HttpProxyAgent(withScheme,  { keepAlive: false });
    }
    useCookieInterceptors = true;
  } else {
    const agents = _buildAgents(jar);
  ...
  // Each cloned client gets a FRESH browser fingerprint (UA, sec-ch-ua, platform).
  // This prevents Discord from correlating multiple bots by fingerprint.
  const uaInfo         = _pickUA();
  const buildNumber    = baseClient.buildNumber;
  const superPropsB64  = _encodeSuperProps(buildNumber, uaInfo);
  ...
  return {
    ...
    http,                              // new axios instance with new proxy + new fingerprint
    uaInfo,                            // FRESH fingerprint — not inherited from parent
    buildNumber,
    superPropsB64,
    fingerprint:     null,             // fresh — not inherited (avoids fingerprint correlation)
...
// ─────────────────────────────────────────────────────────────────────────────
// getClientIp — resolve the public exit IP used by a specific client.
// Uses api.ipify.org (plain JSON, no auth needed). Times out in 8 s.
// Returns the IP string, or null on failure.
// ─────────────────────────────────────────────────────────────────────────────
async function getClientIp(client) {
  try {
    const r = await client.http.get('https://api.ipify.org?format=json', {
      timeout: 8_000,
