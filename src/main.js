...
import { mount as mountActivityFeed } from './utils/activityFeed.js';
...
function toggleNavItems(show) {
  ...
  // Hide the activity feed button when not logged in.
  const afBtn = document.getElementById('activityFeedBtn');
  if (afBtn) afBtn.classList.toggle('af-btn-hidden', !show);
}

const _pageInitialized = new Set();
...
function switchPage(pageId) {
  ...
  const firstVisit = !_pageInitialized.has(pageId);
  _pageInitialized.add(pageId);

  switch (pageId) {
    case 'friends':
      firstVisit ? window.friendsManager.render() : window.friendsManager.refreshFriendsList();
      break;
    case 'servers':
      firstVisit ? window.serverManager.render() : window.serverManager.refreshServersList();
      break;
    case 'dms':
      firstVisit ? window.dmManager.render() : window.dmManager.refreshDMsList();
      break;
    case 'groups':
      firstVisit ? window.groupManager.render() : window.groupManager.refreshGroupsList();
      break;
