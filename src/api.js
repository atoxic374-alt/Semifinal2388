...
// The server stamps every served index.html with the moment it started
// (window.__BUILD_STAMP__). After the server restarts, we periodically
// compare against /api/build-stamp; if they differ, the renderer is
// running stale JS and we surface a "reload required" banner. This is
// what fixes the captcha "invalid-response" loop where mobile browsers
// kept old bundle code despite no-cache headers.
(function watchBuildStamp() {
  ...
  function showBanner() {
    ...
    banner.innerHTML =
      '⚠️ تم تحديث التطبيق — اضغط <u style="cursor:pointer">هنا لإعادة التحميل</u> ' +
      'لتطبيق آخر التحسينات';
  ...
  setTimeout(check, 4000);          // first check shortly after boot
...
window.electronAPI = {
  ...
  readServer:       (id) => testOr(TEST_RESPONSES.ok) || apiCall('POST', `/api/discord/servers/${id}/read`),
  ...
  tsResetBot:        (appId, email, name, icon) => apiCall('POST', `/api/ts/applications/${encodeURIComponent(appId)}/reset-bot-token`, { email, name: name || appId, icon: icon || null }),
  tsGetIntents:      (appId, email) => apiCall('GET', `/api/ts/applications/${encodeURIComponent(appId)}/intents?email=${encodeURIComponent(email)}`),
  tsSetIntents:      (appId, email, enabled) => apiCall('POST', `/api/ts/applications/${encodeURIComponent(appId)}/intents`, { email, enabled }),
  tsApplyIntentsAll: (email, enabled = true) => apiCall('POST', '/api/ts/intents/apply-all', { email, enabled }),
  ...
  tsGetPublic:       (appId, email) => apiCall('GET', `/api/ts/applications/${encodeURIComponent(appId)}/public?email=${encodeURIComponent(email)}`),
  tsSetPublic:       (appId, email, enabled) => apiCall('POST', `/api/ts/applications/${encodeURIComponent(appId)}/public`, { email, enabled }),
  tsApplyPublicAll:  (email, enabled = true) => apiCall('POST', '/api/ts/public/apply-all', { email, enabled }),
