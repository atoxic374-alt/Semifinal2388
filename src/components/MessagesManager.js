...
export class MessagesManager {
    ...
    return `
      <div class="mm-grid">
        ...
        <div class="mm-card mm-span-2 mm-actions-card">
          <button class="mm-btn primary" onclick="window.messagesManager.actionSendNow()">${icon('rocket')} ${t('mm.send_now')}</button>
          <button class="mm-btn warning" onclick="window.messagesManager.actionRepeat()">${icon('repeat')} ${t('mm.start_repeat')}</button>
          <button class="mm-btn success" onclick="window.messagesManager.actionSchedule()">${icon('calendar')} ${t('mm.schedule')}</button>
        </div>
      </div>
    `;
  }
  ...
  renderJobsList() {
    ...
    return this.jobs.map(j => `
      <div class="mm-job-item">
        ...
        <button class="mm-btn danger small" onclick="window.messagesManager.stopJob('${j.id}')">${t('mm.stop')}</button>
      </div>
    `).join('');
  }
  ...
  async actionSendNow() {
    ...
    showNotification(t('mm.sending'));
    try {
      const r = await window.electronAPI.sendMessages(p);
      const ok = (r.results || []).filter(x => x.ok).length;
      const fail = (r.results || []).length - ok;
      showNotification(`${t('mm.sent')} ${t('common.ok')} ${ok}  ${t('common.fail')} ${fail}`);
  ...
  async actionRepeat() {
    ...
    try {
      ...
      if (r.success) showNotification(`${t('mm.job_started')}: ${r.jobId}`); else showNotification(`${t('mm.failed')}: ${r.error}`);
    } catch (e) { showNotification(`${t('mm.failed')}: ${e.message}`); }
  }
  ...
  async actionSchedule() {
    ...
    try {
      ...
      if (r.success) showNotification(`${t('mm.scheduled_in')} ${(r.runIn / 1000) | 0}s`); else showNotification(`${t('mm.failed')}: ${r.error}`);
    } catch (e) { showNotification(`${t('mm.failed')}: ${e.message}`); }
  }
