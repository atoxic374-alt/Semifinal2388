...
export class VoiceManager {
  ...
  _vmBusy(key) { return this._inflight.has(key); }
  _vmLock(key) { this._inflight.set(key, true); }
  _vmUnlock(key) { this._inflight.delete(key); }

  async joinSelected() {
    if (this._vmBusy('join')) { showNotification(t('vm.busy'), 'warning'); return; }
    ...
    finally { this._vmUnlock('join'); }
  }

  async joinAllToSelected() {
    if (this._vmBusy('joinAll')) { showNotification(t('vm.busy'), 'warning'); return; }
    if (!this._needChannel()) return;
    this._vmLock('joinAll');
    try {
    ...
    finally { this._vmUnlock('joinAll'); }
  }

  async leaveSelected() {
    if (this._vmBusy('leave')) { showNotification(t('vm.busy'), 'warning'); return; }
    ...
    finally { this._vmUnlock('leave'); }
  }

  async distributeRandom() {
    if (this._vmBusy('distribute')) { showNotification(t('vm.busy'), 'warning'); return; }
    if (!this.selectedGuild) { showNotification(t('vm.select_server_first'), 'error'); return; }
    if (!this._needAccounts()) return;
    ...
    finally { this._vmUnlock('distribute'); }
  }

  async applyState(stateId) {
    if (this._vmBusy('state')) { showNotification(t('vm.busy'), 'warning'); return; }
  ...
  async startRotation() {
    if (this._vmBusy('rotation')) { showNotification(t('vm.busy'), 'warning'); return; }
    ...
    finally { this._vmUnlock('rotation'); }
  }

  async startCycle() {
    if (this._vmBusy('cycle')) { showNotification(t('vm.busy'), 'warning'); return; }
    if (!this.selectedGuild) { showNotification(t('vm.select_server_first'), 'error'); return; }
    if (!this._needAccounts()) return;
    ...
    finally { this._vmUnlock('cycle'); }
  }

  async stopTask(type, id) {
    const stopKey = `stop_${type}_${id}`;
    if (this._stoppingTasks.has(stopKey)) { showNotification(t('vm.busy'), 'warning'); return; }
    this._stoppingTasks.add(stopKey);
    try {
    ...
    finally { this._stoppingTasks.delete(stopKey); }
  }
