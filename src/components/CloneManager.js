...
export class CloneManager {
  ...
  async snapshot(kind, id) {
    if (this._inflight) { showToast(t('cl.busy'), 'warning'); return; }
    sfx.click();
    if (kind === 'server') {
      const opts = await this.askCaptureOptions();
      if (!opts) return;
      this._inflight = true;
      const log = openOperationLog({ title: t('cl.op_snapshot_title'), total: 1, cancellable: false });
      const k = 'snap';
      log.start(k, { title: t('cl.op_snapshot_title') });
      try {
        ...
        if (!r.success) { sfx.fail(); log.fail(k, { title: t('cl.op_failed'), error: r.error || 'Failed' }); log.summary({ ok: 0, fail: 1, total: 1 }); log.close({ delay: 1500 }); showToast(r.error || 'Failed', 'error'); return; }
        sfx.success();
        log.success(k, { title: t('cl.op_snapshot_done') }); log.summary({ ok: 1, fail: 0, total: 1 }); log.close({ delay: 1200 });
        this.viewing = { snapshot: r.snapshot, name: '' };
        this.selectedChannels = null;
        this.renderViewer();
      } catch (e) { sfx.fail(); log.fail(k, { title: t('cl.op_failed'), error: e.message }); log.summary({ ok: 0, fail: 1, total: 1 }); log.close({ delay: 1500 }); showToast(e.message, 'error'); }
  ...
  renderViewer() {
    ...
    if (s.kind !== 'server') {
      body.querySelector('#cl-paste-wh').addEventListener('click', async () => {
        ...
        this._inflight = true;
        const pasteBtn = body.querySelector('#cl-paste-wh');
        if (pasteBtn) { pasteBtn.disabled = true; pasteBtn.dataset.prevHtml = pasteBtn.innerHTML; pasteBtn.innerHTML = `<span class="dm-mini-spin"></span> …`; }
        const log = openOperationLog({ title: t('cl.op_paste_title'), total: 1, cancellable: false });
        const k = 'paste';
        log.start(k, { title: t('cl.op_paste_title') });
        try {
          ...
          if (r.success) {
            ...
            log.success(k, { title: t('cl.op_paste_done') }); log.summary({ ok: okC, fail: s.messages.length - okC, total: s.messages.length }); log.close({ delay: 1200 });
            if (okC < s.messages.length) showToast(`${t('cl.pasted')}: ${okC}/${s.messages.length}`, 'info');
          } else {
            sfx.fail(); log.fail(k, { title: t('cl.op_failed'), error: r.error || 'Failed' }); log.summary({ ok: 0, fail: 1, total: 1 }); log.close({ delay: 1500 });
            showToast(r.error || 'Failed', 'error');
          }
        } catch (e) {
          sfx.fail(); log.fail(k, { title: t('cl.op_failed'), error: e.message }); log.summary({ ok: 0, fail: 1, total: 1 }); log.close({ delay: 1500 });
          showToast(e.message, 'error');
        } finally {
          this._inflight = false;
          if (pasteBtn) { pasteBtn.disabled = false; pasteBtn.innerHTML = pasteBtn.dataset.prevHtml || pasteBtn.innerHTML; }
        }
      });
    } else {
      this.bindPasteOptions(body, s);
  ...
  bindPasteOptions(body, s) {
    ...
    body.querySelector('#cl-build').addEventListener('click', async (ev) => {
      ...
      if (this._inflight) { showToast(t('cl.busy'), 'warning'); return; }
      this._inflight = true;
      const buildBtn = ev.currentTarget;
      if (buildBtn) { buildBtn.disabled = true; buildBtn.dataset.prevHtml = buildBtn.innerHTML; buildBtn.innerHTML = `<span class="dm-mini-spin"></span> …`; }
      const log = openOperationLog({ title: t('cl.op_build_title'), total: 1, cancellable: false });
      const k = 'build';
      log.start(k, { title: t('cl.op_build_title') });
      try {
        ...
        if (r.success) {
          sfx.success();
          log.success(k, { title: t('cl.op_build_done') }); log.summary({ ok: 1, fail: 0, total: 1 }); log.close({ delay: 1200 });
        ...
        this._inflight = false;
        if (buildBtn) { buildBtn.disabled = false; buildBtn.innerHTML = buildBtn.dataset.prevHtml || buildBtn.innerHTML; }
      }
    });
  }
