...
export class MassFriendManager {
  ...
  _setBtnBusy(sel, busy, origHtml) {
    const btn = this.contentArea.querySelector(sel);
    if (!btn) return;
    btn.disabled = busy;
    if (busy) { btn.dataset.prevHtml = btn.innerHTML; btn.innerHTML = `<span class="dm-mini-spin"></span> …`; }
    else if (btn.dataset.prevHtml) { btn.innerHTML = btn.dataset.prevHtml; delete btn.dataset.prevHtml; }
  }

  async startAdd() {
    if (this._inflight) { showNotification(t('mf.busy'), 'warning'); return; }
    ...
    sfx.click();
    this._inflight = true;
    this._setBtnBusy('#mf-go-add', true);
    try {
      const body = {
    ...
    finally { this._inflight = false; this._setBtnBusy('#mf-go-add', false); }
  }
  ...
  async startRemove() {
    ...
    sfx.click();
    this._inflight = true;
    this._setBtnBusy('#mf-go-remove', true);
    try {
    ...
    finally { this._inflight = false; this._setBtnBusy('#mf-go-remove', false); }
  }
}
